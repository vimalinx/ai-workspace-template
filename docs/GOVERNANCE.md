...
1. **内容环**：在使命和授权范围内新增 Agenda、搜索节点、假设、实验、事件、raw knowledge、债务和派生索引，可以由 Agent 自主完成。
2. **流程环**：Skill/Playbook 必须来自重复成功与失败证据，经过历史回放、新案例验证和 fresh-context review 后才能晋升。
3. **宪法环**：修改 `AGENTS.md` 核心不变量、顶层结构、权威来源、Schema、状态语义、Git 或权限边界时，必须提交 RFC、迁移计划、正负测试、审阅和回滚入口。

运行期 lease 和派生 view 不属于权威治理事实；Agent 崩溃后可以过期或重建。子 Agent Assignment 则属于版本化事实，必须保留目标、作用域、预算、交付和整合责任。
