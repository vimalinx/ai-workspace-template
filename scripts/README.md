...
| 脚本 | 作用 | 是否写入 |
|---|---|---|
| `workspace_protocol.py` | 路由、Mission/Agenda/Search/Hypothesis/Experiment/Evaluation/Assignment、状态迁移、事件、派生索引、租约和交接 | 仅显式变更权威对象；索引写入 `.workspace/views/`，租约写入 `.workspace/runtime/` |
...
自主协议的完整命令见 `docs/CLI-REFERENCE.md`。`validate` 是只读检查；`index rebuild` 只刷新可重建视图。
