...
class Auditor:
    ...
    def check_autonomous_protocol(self) -> None:
        """Validate the optional autonomous control plane without mutating authority."""
        protocol_path = self.root / "governance/protocol.toml"
        if not protocol_path.is_file():
            return
        try:
            from workspace_protocol import ProtocolError, WorkspaceProtocol
            protocol = WorkspaceProtocol(self.root)
            findings = protocol.validate()
        except (ImportError, ProtocolError, OSError, ValueError) as exc:
            self.error(
                "AUTONOMOUS_PROTOCOL_INVALID",
        ...
        for finding in findings:
            severity = finding.severity if finding.severity in {"ERROR", "WARN", "NOTE"} else "ERROR"
            self.add(
                severity,
                f"PROTOCOL_{finding.code}",
                finding.subject,
                finding.message,
                finding.remediation,
                accountable=severity == "WARN",
            )
    ...
    def run(self) -> list[Issue]:
        ...
        self.check_autonomous_protocol()
        self.check_git_hook()
        self.check_debts()
        return self.issues
