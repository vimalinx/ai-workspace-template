...
The generated workspace includes the Autonomous Workspace Protocol: progressive read routes, per-item `.agent/` control planes, deterministic object transitions, subagent Assignments, derived indexes, runtime leases and strict handoffs. Bootstrap installs these contracts; it does not run a permanent Agent or install an external scheduler.
...
Then run `python3 scripts/workspace_protocol.py validate`, rebuild derived indexes, and run the target's relevant tests. For real tests, prompt/model comparisons, migration trials, or consequential decisions, use the nearest project `.ai/` experiment ledger and retain both failed and successful runs.
